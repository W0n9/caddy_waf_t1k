...
| File | Responsibility |
|------|---------------|
| `waf.go` | Module registration, `Engine` wrapper struct, `Provision` (pool init, shared via `caddy.UsagePool`), `ServeHTTP` (detect + block + metrics), `Cleanup`, error classification (`isEngineError`), passive health check (`countFailure`) |
...
**Prometheus metrics:**  
Multiple `waf_chaitin` instances with the same `waf_engine_addr` and pool/health configuration automatically share a single connection pool and health state via `caddy.UsagePool`. Metrics are labeled by engine only (no per-instance distinction); `Cleanup` decrements the shared pool's reference count when an instance is removed. On Caddy reload, the new configuration is provisioned before the old one is cleaned up, so shared pools retain their connections and health state across reloads.
